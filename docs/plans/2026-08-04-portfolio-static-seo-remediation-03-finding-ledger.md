...
| ACC04-EVIDENCE-SELF-REFERENCE | independent Acceptance-04 strict verification | evidence generator and embedded verification receipt | Seal `verification-result.json` containing verifier stdout, then rewrite the manifest to include that receipt | Embedded receipt must bind the exact final package without a stale or impossible self-reference | Receipt recorded penultimate manifest SHA-256 `69566018…`, while the sealed manifest was `ab8744fd…` | High | A verifier result containing the manifest hash cannot be included in that same whole-package manifest without changing the hash it claims | Use a deterministic receipt containing exact verifier identity, binding and status but no self-referential manifest hash; seal once, validate receipt binding, and write no package files after final verification | `EV-R03-WRONG-COMMIT` now requires receipt/manifest binding consistency | RESOLVED BY ACCEPTANCE-04 CORRECTION |
...
| BUILD-REPRO-01 | two clean Windows and two clean Ubuntu/WSL builds | Vite production output | Build the same commit with Bun 1.3.14 on Windows and Linux | Exact path/byte/SHA-256 manifest equality | Each platform was internally stable, but the immediately executed dynamic import caused Vite to emit a preload helper with platform-specific rewrites and a different runtime chunk/hash | High | The WebGL module was imported asynchronously during every boot, so the wrapper added platform variance without providing user-triggered deferred loading | Use a normal static module import while retaining constructor-level WebGL fallback; keep the standard Vite/esbuild toolchain and repeat all four builds | four-way 37-file manifest comparison | RESOLVED |
