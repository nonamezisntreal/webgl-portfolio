...
| BUILD-REPRO-01 | two clean Windows and two clean Ubuntu/WSL builds | Vite/esbuild production output | Build the same commit with Bun 1.3.14 on Windows and Linux | Exact path/byte/SHA-256 manifest equality | Each platform was internally stable, but esbuild applied different identifier and syntax rewrites and changed one runtime chunk/hash | High | Identifier naming and selected syntax folds differ between platform binaries even from identical sources | Disable `esbuild.minifyIdentifiers` and `esbuild.minifySyntax`; retain whitespace minification and repeat all four builds | four-way 38-file manifest comparison | RESOLVED |
